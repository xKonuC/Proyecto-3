...
const ThesisRegistrationTable = memo((props) => {
    ...
    const tbodyContent = (
        <TbodyContent itemsLength={items.length} isLoading={isLoading} length={options.length}>
            {currentItems.map((item) => (
                <tr key={item.thesisRegistrationID} className={tbodyContentTr}>
                    <CheckboxCell id={`deleteInput-${item.thesisRegistrationID}`} checked={selectedItems.some((selectedItem) => selectedItem.thesisRegistrationID === item.thesisRegistrationID)} onChange={(e) => handleCheckboxChange(e, setSelectedItems, 'thesisRegistrationID', item)} />
                    {options.map((option) => (
                        (option.value === 'title') ? (
                            <td key={option.value} className='break-words px-0 py-2 w-96'>
...
export default ThesisRegistrationTable;