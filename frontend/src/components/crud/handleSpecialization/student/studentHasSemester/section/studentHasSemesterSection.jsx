...
import { clearNewStudentHasSemester, setNewStudentHasSemester } from '../../../../../../redux/slice/handleSpecialization/student/studentHasSemester/studentHasSemesterSlice';
...
import StudentHasSemesterForm from '../form/studentHasSemesterForm'
import TimeIndicator from '../../../../../shared/timeIndicator';
import FormButtons from '../../../../../button/form/formButtons';
import ThesisRegistrationCRUD from '../../thesisRegistration/thesisRegistrationCRUD';

const StudentHasSemesterSection = ({ items, url, itemName, permission, isLoading, showAlert, responseHandler }) => {
    ...
    const handleEdit = (item) => {
        setUpdateId(item.studentHasSemesterID);
        dispatch(setNewStudentHasSemester({
    ...
    const clearItem = () => {
        dispatch(clearNewStudentHasSemester());
        setUpdateId(null);
    };
    ...
    const handleSubmit = async (event) => {
        ...
        if (updateId !== null) {
            ...
            await updateService.execute({ studentHasSemesterID: updateId, ...newStudentHasSemester });
            clearItem();
        } else {
    ...
    const getStatusClass = (evaluationStatusID, evaluationTypeID) => {
        if (evaluationTypeID === 1 && evaluationStatusID === 5) {
            return 'bg-green-500';
        }
            ...
            <div className="container flex flex-col">
                <div className="divide-y divide-orange-second">
                        ...
                        <div className={`${isLoading ? 'animate-pulse' : ''} relative grid justify-center grid-cols-5 py-2 sm:py-4 mx-auto sm:space-y-4 space-y-0`}>
                            ...
                            <div className="flex flex-col justify-center text-center col-span-full sm:col-span-4 sm:text-left">
                                <div className='mt-2 mb-1 space-y-1'>
                                    <StudentHasSemesterForm updateId={updateId} />
                                </div>
                    ...
                    {isLoading ?
                        ...
                        <>
                            {items.map((item) => (
                                <div key={item.studentHasSemesterID}
                                    ...
                                    <div className="flex flex-col justify-center text-center col-span-full sm:col-span-4 sm:text-left">
                                        <div className="flex items-center justify-between flex-col sm:flex-row">
                                            <div>
                                                <span className={`${(updateId && updateId === item.studentHasSemesterID) ? 'hidden' : 'flex'} text-sm sm:text-base font-normal text-orange-main`}>
                                                    {item.semester.year} - {item.semester.semesterNumber === 1 ? 'Primer' : 'Segundo'} Semestre
                                                </span>
                                            </div>
                                            <div className={`text-sm sm:text-base px-2 py-1 mt-1 sm:mt-0 font-bold rounded ${getStatusClass(item.evaluationStatusID, item.evaluationTypeID)} text-white`}>
                                                {item.evaluationStatus.name ? item.evaluationStatus.name : 'Aún no ha subido su Informe'}
                                            </div>
                                        </div>
                                        <div className={`${(updateId && updateId === item.studentHasSemesterID) ? '' : 'hidden'} mt-2 mb-1 space-y-1`}>
                                            <StudentHasSemesterForm updateId={updateId} />
                                        </div>
                                        <span className={`${(updateId && updateId === item.studentHasSemesterID) ? 'hidden' : 'flex'} justify-center sm:justify-start text-2xl md:text-3xl font-bold mb-2`}>{item.specialization.name} - {item.evaluationType.name}</span>
                                        {item.evaluationStatus.name && (
                                            <span className="break-words my-1.5 text-sm sm:text-base text-gray-500">
                                                {item.evaluationStatus.description}
                                            </span>
                                        )}
                                        {(updateId && updateId === item.studentHasSemesterID) && (
                                            <div className='mt-1'>
                                                <FormButtons
                                        ...
                                        <div className={`${(updateId && updateId === item.studentHasSemesterID) ? 'hidden' : 'flex'} flex-col sm:flex-row gap-1 sm:gap-2 h-10`}>
                                        ...
                                        <div className={`${(item.evaluationTypeID === 2) ? 'flex' : 'hidden'} flex-col sm:flex-row gap-1 sm:gap-2 h-10 mt-1`}>

                                            <ThesisRegistrationCRUD />
                                        </div>
                                        <div className={`${(updateId && updateId === item.studentHasSemesterID) ? 'hidden' : 'flex'} flex-col sm:flex-row gap-1 sm:gap-2 h-10 mt-1`}>
                                            <div className='flex-1'>
                                                <Link
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    to={`/Dashboard/Specialization/EvaluationProcess/${item.evaluationTypeID}/${item.studentHasSemesterID}`}
                                                >
                                                    <StyledButton >
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5" />                                                </svg>
                                                        {item.evaluationTypeID === 1 ? 'Ir a Portal de Anteproyecto' : 'Ir a Portal de Tesis'}
                                                    </StyledButton>
                                                </Link>
                                            </div>
                                        </div>
...
export default StudentHasSemesterSection;
