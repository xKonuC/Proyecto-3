...
const GraduateTable = (props) => {
  ...
  const theadContent = (
    <>
      ...
      <th className={theadContentDiv}>Año Graduación</th>
      <th className={theadContentDiv}>Teléfono</th>
      <th className={theadContentDiv}>Email Personal</th>
      <th className={theadContentDiv}>Lugar Trabajo</th>
      <th className={theadContentDiv}>Cargo</th>
      <th className={theadContentDiv}>Grupo</th>
      <th className={theadContentDiv}>Articulation</th>
    </>
  );

  const tbodyContent = (
    <TbodyContent itemsLength={Array.isArray(currentItems) ? currentItems.length : 0} length={13} isLoading={false}>
          ...
          <tr key={item.userID} className={tbodyContentTr}>
            <CheckboxCell
              ...
              onChange={(e) => {
                ...
                if (e.target.checked) newSelectedItems = [...selectedItems, item.userID];
                else newSelectedItems = selectedItems.filter((id) => id !== item.userID);
                setSelectedItems(newSelectedItems);
                setSelectAll(newSelectedItems.length === currentItems.length);
              }}
            />

            <ItemCell value={item.rut || 'N/A'} />
            ...
            <ItemCell value={item.graduationYear || 'No disponible'} />
            <ItemCell value={item.phone || 'N/A'} />
            <ItemCell value={item.personalEmail || 'N/A'} />
            <ItemCell value={item.workPlace || 'N/A'} />
            <ItemCell value={item.job || 'N/A'} />
            <ItemCell value={item.group || 'N/A'} />
            <ItemCell value={item.articulation || 'N/A'} />

            <ActionsCell>
              <div className="flex gap-1">
                <StyledButton onClick={() => onUpdate(item.userID)} className={tbodyContentButton} title="Editar">
                  <EditIcon />
                </StyledButton>
              </div>
            </ActionsCell>
        ...
        <div>
          <EnhancedTable
            ...
            onChange={(e) => {
              ...
              setSelectedItems(e.target.checked ? currentItems.map((it) => it.userID) : []);
            }}
          />
        </div>
      </div>
