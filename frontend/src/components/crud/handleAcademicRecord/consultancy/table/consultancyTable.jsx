...
const ConsultancyTable = memo((props) => {
    ...
    const tbodyContent = (
        <TbodyContent itemsLength={items.length} isLoading={isLoading} length={options.length}>
            {currentItems.map((item) => (
                <tr key={item.consultancyID} className={tbodyContentTr}>
                    <CheckboxCell id={`deleteInput-${item.consultancyID}`} checked={selectedItems.some((selectedItem) => selectedItem.consultancyID === item.consultancyID)} onChange={(e) => handleCheckboxChange(e, setSelectedItems, 'consultancyID', item)} />
                    {options.map((option) => (
                        (option.value === 'objective' || option.value === 'title') ? (
