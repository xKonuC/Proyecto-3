...
import PatentForm from './form/patentForm';
...
const PatentCRUD = ({ name, urls, title, subtitle }) => {
  ...
  const handleDeleteSelected = async () => {
    ...
    await deleteService.execute({userID}, selectedItems, 'patentID', 'patentIDs', MAX_LENGTH_ARRAY_NUMBER);
  };
  ...
  const handleData = (data) => {
    const sortedItems = sortItems(data, 'patentID');
  ...
  const handleEdit = (item) => {
    ...
    setUpdateId(item.patentID);
    dispatch(setNewItem({
      inventors: item.inventors,
      patentName: item.patentName,
      applicationDate: applicationDate.toISOString().split('T')[0] || null,
      publicationDate: publicationDate.toISOString().split('T')[0] || null,
  ...
  return (
    <>
      ...
      {/* Componente para mostrar mensajes de alerta */}
      <PatentForm url={urls[0]} updateId={updateId} itemName={name} showAlert={showAlert} modalOpen={modalOpen} closeModal={closeModal} responseHandler={responseHandler} />
