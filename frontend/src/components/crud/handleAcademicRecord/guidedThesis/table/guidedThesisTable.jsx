...
const GuidedThesisTable = memo((props) => {
    ...
    const tbodyContent = (
        <TbodyContent itemsLength={items.length} isLoading={isLoading} length={options.length}>
            {currentItems.map((item) => (
                <tr key={item.guidedThesisID} className={tbodyContentTr}>
                    <CheckboxCell id={`deleteInput-${item.guidedThesisID}`} checked={selectedItems.some((selectedItem) => selectedItem.guidedThesisID === item.guidedThesisID)} onChange={(e) => handleCheckboxChange(e, setSelectedItems, 'guidedThesisID', item)} />
                    {options.map((option) => (
                        option.value === 'title' ? (
...
export default GuidedThesisTable;