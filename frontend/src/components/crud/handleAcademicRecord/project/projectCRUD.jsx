...
import {
  ...
  clearNewItem,
  clearFilteredItems,
} from '../../../../redux/slice/handleAcademicRecord/project/projectSlice';
...
const ProjectCRUD = ({ name, urls, title, subtitle }) => {
  ...
  const handleDeleteSelected = async () => {
    ...
    await deleteService.execute({userID}, selectedItems, 'projectID', 'projectIDs', MAX_LENGTH_ARRAY_NUMBER);
  };
  ...
  const handleData = (data) => {
    const sortedItems = sortItems(data, 'projectID');
  ...
  return (
    <>
      ...
      {/* Componente para mostrar mensajes de alerta */}
      <ProjectForm url={urls[0]} updateId={updateId} itemName={name} showAlert={showAlert} modalOpen={modalOpen} closeModal={closeModal} responseHandler={responseHandler} />
