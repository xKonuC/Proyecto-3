    ...
    const tbodyContent = (
        <TbodyContent itemsLength={items.length} isLoading={isLoading} length={options.length}>
            {currentItems.map((item) => (
                <tr key={item.projectID} className={tbodyContentTr}>
                    <CheckboxCell id={`deleteInput-${item.projectID}`} checked={selectedItems.some((selectedItem) => selectedItem.projectID === item.projectID)} onChange={(e) => handleCheckboxChange(e, setSelectedItems, 'projectID', item)} />
                    {options.map((option) => (
                        option.value === 'title' ? (
