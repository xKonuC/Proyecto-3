...
import {
  ...
  clearNewItem,
  clearFilteredItems,
} from '../../../../redux/slice/handleAcademicRecord/bookChapter/bookChapterSlice';
...
import BookChapterForm from './form/bookChapterForm';
import BookChapterTable from './table/bookChapterTable';
...
const BookChapterCRUD = ({ name, urls, title, subtitle }) => {
  ...
  const handleDeleteSelected = async () => {
    ...
    await deleteService.execute({userID}, selectedItems, 'bookChapterID', 'bookChapterIDs', MAX_LENGTH_ARRAY_NUMBER);
  };
  ...
  const handleData = (data) => {
    const sortedItems = sortItems(data, 'bookChapterID');
  ...
  const handleEdit = (item) => {
    setUpdateId(item.bookChapterID);
    dispatch(setNewItem({
  ...
  return (
    <>
      ...
      {/* Componente para mostrar mensajes de alerta */}
      <BookChapterForm url={urls[0]} updateId={updateId} itemName={name} showAlert={showAlert} modalOpen={modalOpen} closeModal={closeModal} responseHandler={responseHandler} />
...
export default BookChapterCRUD;