...
const BookChapterTable = memo((props) => {
    ...
    const tbodyContent = (
        <TbodyContent itemsLength={items.length} isLoading={isLoading} length={options.length}>
            {currentItems.map((item) => (
                <tr key={item.bookChapterID} className={tbodyContentTr}>
                    <CheckboxCell id={`deleteInput-${item.bookChapterID}`} checked={selectedItems.some((selectedItem) => selectedItem.bookChapterID === item.bookChapterID)} onChange={(e) => handleCheckboxChange(e, setSelectedItems, 'bookChapterID', item)} />
                    {options.map((option) => (
                        (option.value === 'authors' || option.value === 'bookName' || option.value === 'chapterName') ? (
...
export default BookChapterTable;