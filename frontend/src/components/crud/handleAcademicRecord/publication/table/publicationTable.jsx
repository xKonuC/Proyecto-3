    ...
    const tbodyContent = (
        <TbodyContent itemsLength={items.length} isLoading={isLoading} length={options.length}>
            {currentItems.map((item) => (
                <tr key={item.publicationID} className={tbodyContentTr}>
                    <CheckboxCell id={`deleteInput-${item.publicationID}`} checked={selectedItems.some((selectedItem) => selectedItem.publicationID === item.publicationID)} onChange={(e) => handleCheckboxChange(e, setSelectedItems, 'publicationID', item)} />
                    {options.map((option) => (
                        (option.value === 'authors' || option.value === 'journal' || option.value === 'title') ? (
