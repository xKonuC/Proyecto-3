...
import {
  ...
  clearNewItem,
  clearFilteredItems,
} from '../../../../redux/slice/handleAcademicRecord/publication/publicationSlice';
...
import PublicationTable from './table/publicationTable';
...
const PublicationCRUD = ({ name, urls, title, subtitle }) => {
  ...
  const handleDeleteSelected = async () => {
    ...
    await deleteService.execute({userID}, selectedItems, 'publicationID', 'publicationIDs', MAX_LENGTH_ARRAY_NUMBER);
  };
  ...
  const handleData = (data) => {
    const sortedItems = sortItems(data, 'publicationID');
  ...
  const handleEdit = (item) => {
    setUpdateId(item.publicationID);
    dispatch(setNewItem({
  ...
  return (
    <>
      ...
      {/* Componente para mostrar mensajes de alerta */}
      <PublicationForm url={urls[0]} updateId={updateId} itemName={name} showAlert={showAlert} modalOpen={modalOpen} closeModal={closeModal} responseHandler={responseHandler} />
