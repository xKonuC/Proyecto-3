...
import {
  ...
  clearNewItem,
  clearFilteredItems,
} from '../../../../redux/slice/handleTitle/academicHasTitle/academicHasTitleSlice';
...
import AcademicHasTitleForm from './form/academicHasTitleForm';
import AcademicHasTitleTable from './table/academicHasTitleTable';
...
const AcademicHasTitleCRUD = ({ name, urls, title, subtitle }) => {
  ...
  const handleDeleteSelected = async () => {
    ...
    await deleteService.execute({ userID }, selectedItems, 'academicHasTitleID', 'academicHasTitleIDs', MAX_LENGTH_ARRAY_NUMBER);
  };
  ...
  const handleData = (data) => {
    ...
    const sortedItems = sortItems(data, 'academicHasTitleID');
  ...
  const handleEdit = (item) => {
    setUpdateId(item.academicHasTitleID);
    dispatch(setNewItem({
  ...
  useEffect(() => {
    ...
    if (userID || !location.pathname.includes('/Administrator/')) {
      setIsLoading(true);
  ...
  return (
    <>
      ...
      {/* Componente para mostrar mensajes de alerta */}
      <AcademicHasTitleForm selectedFile={selectedFile} selectTitle={selectTitle} url={urls[0]} updateId={updateId} itemName={name} setSelectedFile={setSelectedFile} showAlert={showAlert} modalOpen={modalOpen} closeModal={closeModal} responseHandler={responseHandler} />
      ...
      <ScreenWrapper>
        ...
        {/* Tabla de Títulos de Académicos */}
        <AcademicHasTitleTable
          isLoading={isLoading}
...
export default AcademicHasTitleCRUD;