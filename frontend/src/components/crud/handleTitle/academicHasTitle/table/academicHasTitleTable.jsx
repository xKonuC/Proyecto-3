...
const options = [
    ...
    { label: `ID`, value: 'academicHasTitleID' },
...
const AcademicHasTitleTable = memo(({ isLoading, selectedItems, selectAll, setSelectedItems, setSelectAll, handleEdit }) => {
    const { items, filteredItems } = useSelector((state) => state.handleTitle.academicHasTitle);
    ...
    const tbodyContent = (
        <TbodyContent itemsLength={items.length} isLoading={isLoading} length={options.length}>
            {currentItems.map((item) => (
                <tr key={item.academicHasTitleID} className={tbodyContentTr}>
                    <CheckboxCell id={`deleteInput-${item.academicHasTitleID}`} checked={selectedItems.some((selectedItem) => selectedItem.academicHasTitleID === item.academicHasTitleID)} onChange={(e) => handleCheckboxChange(e, setSelectedItems, 'academicHasTitleID', item)} />
...
export default AcademicHasTitleTable;