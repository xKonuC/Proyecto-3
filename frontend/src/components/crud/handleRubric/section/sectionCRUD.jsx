...
import {
  ...
  clearNewItem,
  clearFilteredItems,
} from '../../../../redux/slice/handleRubric/section/sectionSlice';
...
const SectionCRUD = ({ name, urls, title, subtitle }) => {
  ...
  const handleEnableSelected = async () => {
    ...
    await updateService.execute({}, selectedItems, 'sectionID', 'sectionIDs', MAX_LENGTH_ARRAY_NUMBER);
  };
  ...
  const handleData = (data) => {
    const sortedItems = sortItems(data, 'sectionID');
  ...
  return (
    <>
      ...
      <Suspense>
        <SectionForm updateId={updateId} url={urls[0]} itemName={name} showAlert={showAlert} modalOpen={modalOpen} closeModal={closeModal} responseHandler={responseHandler} />
      </Suspense>
