...
import {
  ...
  clearNewItem,
  clearFilteredItems,
} from '../../../../redux/slice/handleRubric/question/questionSlice';
...
const QuestionCRUD = ({ name, urls, title, subtitle }) => {
  ...
  const handleEnableSelected = async () => {
    ...
    await updateService.execute({}, selectedItems, 'questionID', 'questionIDs', MAX_LENGTH_ARRAY_NUMBER);
  };
  ...
  const handleData = (data) => {
    const sortedItems = sortItems(data, 'questionID');
  ...
  return (
    <>
      ...
      <Suspense>
        <QuestionForm updateId={updateId} url={urls[0]} itemName={name} showAlert={showAlert} modalOpen={modalOpen} closeModal={closeModal} responseHandler={responseHandler} />
      </Suspense>
