...
const RubricManagement = ({ evaluationTypeID, categoryEvaluator, items, itemsCopy, info, setItems, handleSaveChanges, showClearButton }) => {
    ...
    const handleClearRubric = () => {
        ...
        if (items && typeof items === 'object') {
            ...
            const modifiedRubric = { ...items };
            // Eliminar la propiedad sections y templateID del objeto modificado
            modifiedRubric.sections = [];
            modifiedRubric.templateID = null;
            // Devolver el objeto modificado
            dispatch(setItems.setItems(modifiedRubric));
        } else {
    ...
    const resetRubricToInitialState = () => {
        // Verificar si rubric es un objeto válido
        if (itemsCopy && typeof itemsCopy === 'object') {
            ...
            const modifiedRubric = { ...itemsCopy };
            dispatch(setItems.setItems(modifiedRubric));
        } else {
            // Si rubric no es un objeto válido, devolver un objeto vacío
            dispatch(setItems.setItems({}));
            ...
            <ModalFile isOpen={modalOpenFile} onClose={closeModalFile}>
                <div className="w-full h-screen">
                    <PDFViewer width="100%" height="100%">
                        <ExportEvaluationPDF rubric={items} rubricInfo={info} evaluationTypeID={evaluationTypeID} categoryEvaluator={categoryEvaluator}/>
                    </PDFViewer>
                </div>
            </ModalFile>
