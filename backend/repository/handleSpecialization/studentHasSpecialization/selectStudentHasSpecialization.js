...
class SelectStudentHasSpecialization {
  async selectStudentHasSpecialization() {
    const connection = await pool.selectConnection();
    ...
    LEFT JOIN semester se1 ON shs.entrySemesterID = se1.semesterID 
    LEFT JOIN semester se2 ON shs.completionSemesterID = se2.semesterID where userID = ?`,
    );
    connection.release();
    return { result };
  }
...
export { SelectStudentHasSpecialization };
